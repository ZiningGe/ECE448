...
def compute_tf_idf(train_set, train_labels, dev_set):
    ...
    for email in train_set:
        for word in Counter(email).keys():
    ...
    for email in dev_set:
        c = Counter(email)
        total_word = sum(c.values())
        max_tfidf = 0
        max_word = ""
        for word in c.keys():
            if word in doc_count:
                tfidf = (c[word] / total_word) * np.log(len(train_set) / (1 + doc_count[word]))
            else:
                tfidf = (c[word] / total_word) * np.log(len(train_set) / (1 + 0))
            if tfidf > max_tfidf:
                max_tfidf = tfidf
                max_word = word
        retval.append(max_word)
